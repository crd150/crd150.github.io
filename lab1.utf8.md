...
We learned that data values can be stored in data objects known as vectors.  The next step is to learn how to store vectors into an even higher level data structure.  The data frame can do this.   Data frames store vectors of the same length.  Create a vector called "v2"" storing the values 5, 12, and 25
