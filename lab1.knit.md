...
This lab guide will follow closely and supplement the material presented in Chapters X in R for Data Science (RDS), which is available free at http://r4ds.had.co.nz/index.html

<p class="comment", style="font-style:normal">**Assignment 1 is due by 11:59 pm, October 5th on Canvas.**  See X for assignment guidelines.  You must submit an `.Rmd` file and its associated `.html` file. Name the files: yourLastName_firstInitial_asgnXX. For example: brazil_n_asgn01</p>
...
<center>
![Figure 1: RStudio Interface.](/Users/noli/Documents/UCD/teaching/CRD150/Lab/lab1/rgui.png)

</center>

\

Now may be a good time to read through the class assignment guidelines as they detail what is a R Markdown file.  After you read through the guidelines, open up an R Markdown file.  You should now see four windows in RStudio.
...
* **Environment, History, and Connections tabs** (upper-right)
    ...
    + **History** - shows a list of executed commands in current session.
...
You just learned that R has three basic data types. Now, let's go through how we can store data in R. You do this by using R's various data structures. A vector is the most common and basic data structure and is pretty much the workhorse of R. A vector is simply a sequence of values which can be of any data type.  There are a number of ways to create a vector depending on the data type, but the most common is to insert the data you want to save in a vector into the command `c()`.  For example, represent the values 4, 16 and 9 in a vector
...
You should see the objects b and v1 pop up in the Environment tab on the top right window.  Note that the name "v1"" is nothing special here. You could have named the object x or crd150 or your pet's name.  You can't, however, name objects using special characters (e.g. !, @, $) or only numbers (although you can combine numbers and letters, but a number cannot be at the beginning e.g. 2d2).  For example, you'll get an error if you save the vector `c(4,16,9)` to an object with the following names
...
Every vector has two key properties: *type* and *length*.  The type intuitively indicates the data type that the vector is holding. Use the command `typeof()` to determine the type 
...
We learned that data values can be stored in data objects known as vectors.  The next step is to learn how to store vectors into a even higher level data structure.  The data frame can do this.   Data frames store vectors of the same length.  Create a vector called "v2"" storing the values 5, 12, and 25
...
You can find the number of rows and columns that a data frame has by using the command `dim()`
...
This dataset is a tibble. Let's save it as a data frame by using the `as.data.frame()` function
...
Not all functions work with tibbles, particularly those that are specific to spatial data. As such, we'll be using a combination of tibbles and data frames throughout the class, with a preference towards tibbles where possible.
