...
date: <h4 style="font-style:normal">February 15th, 2023</h4>
output: 
  html_document:
    toc: true
    toc_depth: 3
...
<p class="comment">**Assignment 6 is due by 2:00 pm, February 22nd on Canvas.**  See [here](https://crd150.github.io/hw_guidelines.html) for assignment guidelines. You must submit an `.Rmd` file and its associated `.html` file. Name the files: yourLastName_firstInitial_asgn06. For example: brazil_n_asgn06.</p>
...
Next, let's bring in the Sacramento City boundary using `places()` from the **tigris** package, which we first learned about in [Lab 3](https://crd150.github.io/lab3.html#Using_the_Census_API). Remember, since we brought in 2016-2020 ACS data above, you need to specify `year = 2020` in the `places()` function.
...
The average number of neighbors (i.e. adjacent tracts) is 5.68s, 1 tract has a low of 1 neighbor and 1 has a high of 12 neighbors.
