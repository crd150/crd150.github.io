...
* Component 1: Show the appropriate results for a given question (e.g. map, table, statistics)
* Component 2: Show the appropriate code producing the results. 
* Component 3: Provide correct written answers.  

The expectation for each assignment is to get through as much of it as possible during lab time.  If you are unable to finish the guide during the lab, please complete it on your own.  You can ask for help from TAs and work with other students.  However, you *must* submit your own assignment results.  It is important that you do not skip assignments as class material build on one another. 
...
To be clear, R is a *programming language*. RStudio is an *application*. RMarkdown is a *markup syntax* to convert R script and text into a word, pdf or html document.
...
An assignmnent `.Rmd` template can be found [here](https://raw.githubusercontent.com/crd150/data/master/assignment_template.Rmd).  You can use this template for every assignment or follow the directions below.

To create a new R Markdown file, select File -> New File -> R Markdown from the menu up top.  You should see a window pop up that looks like the following figure.  
...
For each assignment, enter "Assignment #" in *Title*, replacing # with the Assignment number.  For *Author*, enter your name.  Keep *Document* in the left panel and the radio button next to *HTML* in the bottom panel selected.  You should see an RMarkdown file pop up on the top left portion of your RStudio interface.

<center>

![Figure 2: R Markdown window.](/Users/noli/Documents/UCD/teaching/CRD150/Lab/crd150.github.io/hwguide2.png)

</center>
\


You won't need anything including and below "### RMarkdown" - just delete that text.  Instead, you will fill it in with your assignment answers.  Name the file: yourLastName_firstInitial_weekXX.Rmd example: brazil_n_week01.Rmd
...
Your RMarkdown assignment file will contain R code and text.  R code chunks are where you store R code that will be executed. They are inserted in between `` ```{r} `` and `` ``` ``.  For example, to designate `1+1` as R code, it will look like the following in your R Markdown document
...
<center>
![](/Users/noli/Documents/UCD/teaching/CRD150/Lab/crd150.github.io/hwguide5.png)
</center>
\
...
Of course, you'll need to test code to make sure it is running properly.  Rather than writing and running  the code for an assignment at one time, run them in chunks.  To run code in just one chunk, either click on ![](/Users/noli/Documents/UCD/teaching/CRD150/Lab/crd150.github.io/hwguide3.png) and select *Run Current Chunk* or click on ![](/Users/noli/Documents/UCD/teaching/CRD150/Lab/crd150.github.io/hwguide4.png) located on the right corner of the chunk.
...
So, the proper workflow for each assignment will be

1. Go through the step-by-step lab guide.  
2. Either use the RMarkdown assignment template linked above or open up your own RMarkdown document
2. In this documnent, answer all the assignment questions located at the end of the guide
* Most of the questions will ask you to run code.  Show that code in an RMarkdown chunk.  
+ Run that code one chunk at a time to make sure it is working
...
5. Smile, pat yourself on the back, and eat some ice cream
