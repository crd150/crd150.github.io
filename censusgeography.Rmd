...
sac.tracts.metro <- ms_clip(ca.tracts, sac.metro, remove_slivers = TRUE) 
sac.tracts.city <- ms_clip(ca.tracts, sac.city, remove_slivers = TRUE) 
sac.tracts.county <- tracts(state ="CA", county = "Sacramento", cb = TRUE)
...
sac.map <- leaflet(sac.tracts.metro) %>%
  # Base groups
  addTiles(urlTemplate = "http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png") %>%
  # Overlay groups
  addPolygons(color="grey", group = "Metro", weight = 1, smoothFactor = 0.5,
    opacity = 1.0, fillOpacity = 0.5) %>%
  addPolygons(data = sac.tracts.county, color = "blue", group = "County", weight = 1, smoothFactor = 0.5,
    opacity = 1.0, fillOpacity = 0.5) %>%
  addPolygons(data = sac.tracts.city, color = "red", group = "City", weight = 1, smoothFactor = 0.5,
    opacity = 1.0, fillOpacity = 0.5) %>%
  # Layers control
  addLayersControl(
...
sac.map <- leaflet(sac.tracts.metro) %>%
  ...
  addPolygons(color="grey", group = "Metro", weight = 1, smoothFactor = 0.5, 
              opacity = 1.0, fillOpacity = 0.5) %>%
  addPolygons(data = sac.tracts.city, color = "red", group = "City", weight = 1, 
              smoothFactor = 0.5, opacity = 1.0, fillOpacity = 0.5) %>%
  addPolygons(data = sac.tracts.county, color = "blue", group = "County", weight = 1, 
              smoothFactor = 0.5, opacity = 1.0, fillOpacity = 0.5) %>%
  # Layers control
  addLayersControl(
    overlayGroups = c("Metro", "City", "County"),
...
davis.map <- leaflet(davis.zip) %>%
  ...
  # Layers control
  addLayersControl(
    overlayGroups = c("Zipcode", "Tract", "Block"),
    options = layersControlOptions(collapsed = FALSE)
  )
davis.map
```
...
ca.tracts.2000 <- tracts(state ="CA", cb = TRUE, year = 2000)
# Need to reproject 2000 tracts to 2010 CRS
ca.tracts.2000<-st_transform(ca.tracts.2000, crs=st_crs(ca.tracts))
