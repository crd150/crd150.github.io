...
````
`r ''`download.file(url = "https://raw.githubusercontent.com/crd150/data/master/assign8files.zip", destfile = "assign8files.zip")
...
````
`r ''`capitaltracts <- st_read("capitaltracts.shp", stringsAsFactors = FALSE)
````
...
````
`r ''`state <- read_csv("https://raw.githubusercontent.com/crd150/data/master/lihtccapital.csv")
````
